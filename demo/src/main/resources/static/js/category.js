...
let editButtons = document.getElementsByClassName("edit-category");

for (let i = 0; i < editButtons.length; i++) {
  editButtons[i].addEventListener("click", function () {
    let customData = this.getAttribute("data-custom-data");
    fetchEditCategory(customData)
      .then((data) => {
        createEditCategoryForm(data.categoryId, data.name);
      })
      .catch((error) => {
...
function fetchEditCategory(categoryId) {
  return fetch(`/api/v1/edit/category/${categoryId}`).then((response) =>
    response.json()
  );
}

function createEditCategoryForm(categoryId, name) {
  ...
  h2Element.textContent = "カテゴリー編集画面";
  ...
  let baseUrl = "/edit/category/";
  form.action = baseUrl + categoryId;
  form.method = "post";

  createInputField("text", "editCategoryName", "カテゴリー", name, true, form);
